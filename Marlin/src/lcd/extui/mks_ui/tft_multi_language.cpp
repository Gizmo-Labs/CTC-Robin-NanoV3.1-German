...
void machine_setting_disp() {
  if (gCfgItems.language == LANG_GERMAN) {
    MachinePara_menu.title              = MACHINE_PARA_TITLE_DE;
    MachinePara_menu.MachineSetting     = MACHINE_TYPE_CNOFIG_DE;
    MachinePara_menu.MotorSetting       = MOTOR_CONFIG_DE;
    MachinePara_menu.leveling           = MACHINE_LEVELING_CONFIG_DE;
    MachinePara_menu.AdvanceSetting     = ADVANCE_CONFIG_DE;

    machine_menu.default_value = DEFAULT_DE;
    machine_menu.next          = NEXT_DE;
    machine_menu.previous      = PREVIOUS_DE;

    machine_menu.MachineConfigTitle = MACHINE_CONFIG_TITLE_DE;
    machine_menu.MachineType        = MACHINE_TYPE_DE;
    machine_menu.Stroke             = MACHINE_STROKE_DE;
    machine_menu.HomeDir            = MACHINE_HOMEDIR_DE;
    machine_menu.EndStopType        = MACHINE_ENDSTOP_TYPE_DE;
    machine_menu.FilamentConf       = MACHINE_FILAMENT_CONFIG_DE;

    machine_menu.MachineTypeConfTitle = MACHINE_TYPE_CONFIG_TITLE_DE;
    machine_menu.xyz                  = MACHINE_TYPE_XYZ_DE;
    machine_menu.delta                = MACHINE_TYPE_DELTA_DE;
    machine_menu.corexy               = MACHINE_TYPE_COREXY_DE;

    machine_menu.StrokeConfTitle = MACHINE_STROKE_CONF_TITLE_DE;
    machine_menu.xStroke         = X_MAX_LENGTH_DE;
    machine_menu.yStroke         = Y_MAX_LENGTH_DE;
    machine_menu.zStroke         = Z_MAX_LENGTH_DE;

    machine_menu.xmin = X_MIN_LENGTH_DE;
    machine_menu.ymin = Y_MIN_LENGTH_DE;
    machine_menu.zmin = Z_MIN_LENGTH_DE;

    machine_menu.HomeDirConfTitle = HOME_DIR_CONF_TITLE_DE;
    machine_menu.xHomeDir         = HOME_DIR_X_DE;
    machine_menu.yHomeDir         = HOME_DIR_Y_DE;
    machine_menu.zHomeDir         = HOME_DIR_Z_DE;
    machine_menu.min              = HOME_MIN_DE;
    machine_menu.max              = HOME_MAX_DE;

    machine_menu.EndstopConfTitle = ENDSTOP_CONF_TITLE_DE;
    machine_menu.xEndstop_min     = MIN_ENDSTOP_X_DE;
    machine_menu.yEndstop_min     = MIN_ENDSTOP_Y_DE;
    machine_menu.zEndstop_min     = MIN_ENDSTOP_Z_DE;
    machine_menu.xEndstop_max     = MAX_ENDSTOP_X_DE;
    machine_menu.yEndstop_max     = MAX_ENDSTOP_Y_DE;
    machine_menu.zEndstop_max     = MAX_ENDSTOP_Z_DE;
    machine_menu.FilamentEndstop  = ENDSTOP_FIL_DE;
    machine_menu.LevelingEndstop  = ENDSTOP_LEVEL_DE;
    machine_menu.opened           = ENDSTOP_OPENED_DE;
    machine_menu.closed           = ENDSTOP_CLOSED_DE;

    machine_menu.FilamentConfTitle   = FILAMENT_CONF_TITLE_DE;
    machine_menu.InLength            = FILAMENT_IN_LENGTH_DE;
    machine_menu.InSpeed             = FILAMENT_IN_SPEED_DE;
    machine_menu.FilamentTemperature = FILAMENT_TEMPERATURE_DE;
    machine_menu.OutLength           = FILAMENT_OUT_LENGTH_DE;
    machine_menu.OutSpeed            = FILAMENT_OUT_SPEED_DE;

    machine_menu.LevelingParaConfTitle   = LEVELING_CONF_TITLE_DE;
    machine_menu.LevelingParaConf        = LEVELING_PARA_CONF_DE;
    machine_menu.TrammingPosConf         = TRAMMING_POS_DE;
    machine_menu.LevelingAutoCommandConf = LEVELING_AUTO_COMMAND_DE;
    machine_menu.LevelingAutoZoffsetConf = LEVELING_AUTO_ZOFFSET_DE;

    machine_menu.LevelingSubConfTitle = LEVELING_PARA_CONF_TITLE_DE;
    machine_menu.AutoLevelEnable      = AUTO_LEVELING_ENABLE_DE;
    machine_menu.BLtouchEnable        = BLTOUCH_LEVELING_ENABLE_DE;
    machine_menu.ProbePort            = PROBE_PORT_DE;
    machine_menu.ProbeXoffset         = PROBE_X_OFFSET_DE;
    machine_menu.ProbeYoffset         = PROBE_Y_OFFSET_DE;
    machine_menu.ProbeZoffset         = PROBE_Z_OFFSET_DE;
    machine_menu.ProbeXYspeed         = PROBE_XY_SPEED_DE;
    machine_menu.ProbeZspeed          = PROBE_Z_SPEED_DE;
    machine_menu.enable               = ENABLE_DE;
    machine_menu.disable              = DISABLE_DE;
    machine_menu.locked               = LOCKED_DE;
    machine_menu.z_min                = Z_MIN_DE;
    machine_menu.z_max                = Z_MAX_DE;

    machine_menu.LevelingSubDeltaConfTitle = DELTA_LEVEL_CONF_TITLE_DE;
    machine_menu.MachineRadius             = DELTA_MACHINE_RADIUS_DE;
    machine_menu.DiagonalRod               = DELTA_DIAGONAL_ROD_DE;
    machine_menu.PrintableRadius           = DELTA_PRINT_RADIUS_DE;
    machine_menu.DeltaHeight               = DELTA_HEIGHT_DE;
    machine_menu.SmoothRodOffset           = SMOOTH_ROD_OFFSET_DE;
    machine_menu.EffectorOffset            = EFFECTOR_OFFSET_DE;
    machine_menu.CalibrationRadius         = CALIBRATION_RADIUS_DE;

    machine_menu.LevelingSubXYZConfTitle = XYZ_LEVEL_CONF_TITLE_DE;

    machine_menu.TemperatureConfTitle = TEMPERATURE_CONF_TITLE_DE;
    machine_menu.NozzleConf           = NOZZLE_CONF_DE;
    machine_menu.HotBedConf           = HOTBED_CONF_DE;
    machine_menu.PreheatTemperConf    = PREHEAT_TEMPER_DE;

    machine_menu.NozzleConfTitle      = NOZZLE_CONF_TITLE_DE;
    machine_menu.NozzleCnt            = NOZZLECNT_DE;
    machine_menu.NozzleType           = NOZZLE_TYPE_DE;
    machine_menu.NozzleAdjustType     = NOZZLE_ADJUST_TYPE_DE;
    machine_menu.NozzleMinTemperature = NOZZLE_MIN_TEMPERATURE_DE;
    machine_menu.NozzleMaxTemperature = NOZZLE_MAX_TEMPERATURE_DE;
    machine_menu.Extrude_Min_Temper   = EXTRUD_MIN_TEMPER_DE;

    machine_menu.HotbedConfTitle      = HOTBED_CONF_TITLE_DE;
    machine_menu.HotbedAjustType      = HOTBED_ADJUST_DE;
    machine_menu.HotbedMinTemperature = HOTBED_MIN_TEMPERATURE_DE;
    machine_menu.HotbedMaxTemperature = HOTBED_MAX_TEMPERATURE_DE;

    machine_menu.MotorConfTitle        = MOTOR_CONF_TITLE_DE;
    machine_menu.MaxFeedRateConf       = MAXFEEDRATE_CONF_DE;
    machine_menu.AccelerationConf      = ACCELERATION_CONF_DE;
    machine_menu.JerkConf              = JERKCONF_DE;
    machine_menu.StepsConf             = STEPSCONF_DE;
    machine_menu.TMCcurrentConf        = TMC_CURRENT_DE;
    machine_menu.TMCStepModeConf       = TMC_STEP_MODE_DE;
    machine_menu.MotorDirConf          = MOTORDIRCONF_DE;
    machine_menu.HomeFeedRateConf      = HOMEFEEDRATECONF_DE;
    machine_menu.PausePosition         = PAUSE_POSITION_DE;
    machine_menu.WifiSettings          = WIFI_SETTINGS_DE;
    machine_menu.HomingSensitivityConf = HOMING_SENSITIVITY_CONF_DE;
    machine_menu.EncoderSettings         = ENCODER_SETTINGS_DE;

    machine_menu.MaxFeedRateConfTitle = MAXFEEDRATE_CONF_TITLE_DE;
    machine_menu.XMaxFeedRate         = X_MAXFEEDRATE_DE;
    machine_menu.YMaxFeedRate         = Y_MAXFEEDRATE_DE;
    machine_menu.ZMaxFeedRate         = Z_MAXFEEDRATE_DE;
    machine_menu.E0MaxFeedRate        = E0_MAXFEEDRATE_DE;
    machine_menu.E1MaxFeedRate        = E1_MAXFEEDRATE_DE;

    machine_menu.AccelerationConfTitle = ACCELERATION_CONF_TITLE_DE;
    machine_menu.PrintAcceleration     = PRINT_ACCELERATION_DE;
    machine_menu.RetractAcceleration   = RETRACT_ACCELERATION_DE;
    machine_menu.TravelAcceleration    = TRAVEL_ACCELERATION_DE;
    machine_menu.X_Acceleration        = X_ACCELERATION_DE;
    machine_menu.Y_Acceleration        = Y_ACCELERATION_DE;
    machine_menu.Z_Acceleration        = Z_ACCELERATION_DE;
    machine_menu.E0_Acceleration       = E0_ACCELERATION_DE;
    machine_menu.E1_Acceleration       = E1_ACCELERATION_DE;

    machine_menu.JerkConfTitle = JERK_CONF_TITLE_DE;
    machine_menu.X_Jerk        = X_JERK_DE;
    machine_menu.Y_Jerk        = Y_JERK_DE;
    machine_menu.Z_Jerk        = Z_JERK_DE;
    machine_menu.E_Jerk        = E_JERK_DE;

    machine_menu.StepsConfTitle = STEPS_CONF_TITLE_DE;
    machine_menu.X_Steps        = X_STEPS_DE;
    machine_menu.Y_Steps        = Y_STEPS_DE;
    machine_menu.Z_Steps        = Z_STEPS_DE;
    machine_menu.E0_Steps       = E0_STEPS_DE;
    machine_menu.E1_Steps       = E1_STEPS_DE;

    machine_menu.TmcCurrentConfTitle = TMC_CURRENT_CONF_TITLE_DE;
    machine_menu.X_Current           = X_TMC_CURRENT_DE;
    machine_menu.Y_Current           = Y_TMC_CURRENT_DE;
    machine_menu.Z_Current           = Z_TMC_CURRENT_DE;
    machine_menu.E0_Current          = E0_TMC_CURRENT_DE;
    machine_menu.E1_Current          = E1_TMC_CURRENT_DE;

    machine_menu.TmcStepModeConfTitle = TMC_MODE_CONF_TITLE_DE;
    machine_menu.X_StepMode           = X_TMC_MODE_DE;
    machine_menu.Y_StepMode           = Y_TMC_MODE_DE;
    machine_menu.Z_StepMode           = Z_TMC_MODE_DE;
    machine_menu.E0_StepMode          = E0_TMC_MODE_DE;
    machine_menu.E1_StepMode          = E1_TMC_MODE_DE;

    machine_menu.MotorDirConfTitle = MOTORDIR_CONF_TITLE_DE;
    machine_menu.X_MotorDir        = X_MOTORDIR_DE;
    machine_menu.Y_MotorDir        = Y_MOTORDIR_DE;
    machine_menu.Z_MotorDir        = Z_MOTORDIR_DE;
    machine_menu.E0_MotorDir       = E0_MOTORDIR_DE;
    machine_menu.E1_MotorDir       = E1_MOTORDIR_DE;
    machine_menu.Invert_0          = INVERT_P_DE;
    machine_menu.Invert_1          = INVERT_N_DE;

    machine_menu.HomeFeedRateConfTitle = HOMEFEEDRATE_CONF_TITLE_DE;
    machine_menu.XY_HomeFeedRate       = X_HOMESPEED_DE;
    machine_menu.Z_HomeFeedRate = Z_HOMESPEED_DE;

    machine_menu.AdvancedConfTitle = ADVANCED_CONF_TITLE_DE;
    machine_menu.PwrOffDection     = PWROFF_DECTION_DE;
    machine_menu.PwrOffAfterPrint  = PWROFF_AFTER_PRINT_DE;
    machine_menu.HaveUps           = HAVE_UPS_DE;
    machine_menu.Z2andZ2Endstop    = Z2_AND_Z2ENDSTOP_CONF_DE;
    machine_menu.EnablePinsInvert  = ENABLE_PINS_CONF_DE;

    machine_menu.Z2ConfTitle     = Z2_AND_Z2ENDSTOP_CONF_TITLE_DE;
    machine_menu.Z2Enable        = Z2_ENABLE_DE;
    machine_menu.Z2EndstopEnable = Z2_ENDSTOP_DE;
    machine_menu.Z2Port          = Z2_PORT_DE;

    machine_menu.EnablePinsInvertTitle = ENABLE_PINS_CONF_TITLE_DE;
    machine_menu.XInvert               = X_ENABLE_PINS_INVERT_DE;
    machine_menu.YInvert               = Y_ENABLE_PINS_INVERT_DE;
    machine_menu.ZInvert               = Z_ENABLE_PINS_INVERT_DE;
    machine_menu.EInvert               = E_ENABLE_PINS_INVERT_DE;

    machine_menu.key_back    = KEY_BACK_DE;
    machine_menu.key_reset   = KEY_REST_DE;
    machine_menu.key_confirm = KEY_CONFIRM_DE;

    machine_menu.PausePosText   = PAUSE_POSITION_DE;
    machine_menu.xPos           = PAUSE_POSITION_X_DE;
    machine_menu.yPos           = PAUSE_POSITION_Y_DE;
    machine_menu.zPos           = PAUSE_POSITION_Z_DE;
    machine_menu.WifiConfTitle  = WIFI_SETTINGS_TITLE_DE;
    machine_menu.wifiMode       = WIFI_SETTINGS_MODE_DE;
    machine_menu.wifiName       = WIFI_SETTINGS_NAME_DE;
    machine_menu.wifiPassWord   = WIFI_SETTINGS_PASSWORD_DE;
    machine_menu.wifiCloud      = WIFI_SETTINGS_CLOUD_DE;
    machine_menu.wifiConfig     = WIFI_SETTINGS_CONFIG_DE;
    machine_menu.wifiEdit       = WIFI_SETTINGS_EDIT_DE;
    machine_menu.wifiConfigTips = WIFI_CONFIG_TIPS_DE;

    machine_menu.OffsetConfTitle = OFFSET_TITLE_DE;
    machine_menu.Xoffset         = OFFSET_X_DE;
    machine_menu.Yoffset         = OFFSET_Y_DE;
    machine_menu.Zoffset         = OFFSET_Z_DE;

    machine_menu.BLTouchLevelingConfTitle = BLTOUCH_LEVELING_TITTLE_DE;
    machine_menu.BLTouchLevelingConf      = BLTOUCH_LEVELING_DE;
    machine_menu.BLTouchInit              = BLTOUCH_INIT_DE;
    machine_menu.BLTouchOffsetpos         = BLTOUCH_ZOFFSETPOS_DE;
    machine_menu.BLTouchOffsetneg         = BLTOUCH_ZOFFSETNEG_DE;
    machine_menu.BLTouchSave              = BLTOUCH_SAVE_DE;
    machine_menu.BLTouchTest              = BLTOUCH_TEST_DE;

    machine_menu.HomingSensitivityConfTitle = HOMING_SENSITIVITY_CONF_TITLE_DE;
    machine_menu.X_Sensitivity              = X_SENSITIVITY_DE;
    machine_menu.Y_Sensitivity              = Y_SENSITIVITY_DE;
    machine_menu.Z_Sensitivity              = Z_SENSITIVITY_DE;
    machine_menu.Z2_Sensitivity             = Z2_SENSITIVITY_DE;

    machine_menu.EncoderConfTitle = ENCODER_CONF_TITLE_DE;
    machine_menu.EncoderConfText              = ENCODER_CONF_TEXT_DE;
  }
...
void disp_language_init() {
  ...
  media_select_menu.title    = MEDIA_SELECT_TITLE_DE;
  media_select_menu.sd_disk  = SD_CARD_TITLE_DE;
  media_select_menu.usb_disk = USB_DRIVE_TITLE_DE;

  machine_menu.key_0     = KEYBOARD_KEY0_DE;
  machine_menu.key_1     = KEYBOARD_KEY1_DE;
  machine_menu.key_2     = KEYBOARD_KEY2_DE;
  machine_menu.key_3     = KEYBOARD_KEY3_DE;
  machine_menu.key_4     = KEYBOARD_KEY4_DE;
  machine_menu.key_5     = KEYBOARD_KEY5_DE;
  machine_menu.key_6     = KEYBOARD_KEY6_DE;
  machine_menu.key_7     = KEYBOARD_KEY7_DE;
  machine_menu.key_8     = KEYBOARD_KEY8_DE;
  machine_menu.key_9     = KEYBOARD_KEY9_DE;
  machine_menu.key_point = KEYBOARD_KEY_POINT_DE;
  machine_menu.negative  = KEYBOARD_KEY_NEGATIVE_DE;
  // wifi-list
  #if ENABLED(MKS_WIFI_MODULE)
    list_menu.title        = TEXT_WIFI_MENU_TITLE_DE;
    list_menu.file_pages   = FILE_PAGES_DE;

    // tips
    tips_menu.joining      = TEXT_WIFI_JOINING_DE;
    tips_menu.failedJoin   = TEXT_WIFI_FAILED_JOIN_DE;
    tips_menu.wifiConected = TEXT_WIFI_WIFI_CONECTED_DE;
  #endif
  machine_setting_disp();

  operation_menu.babystep = TEXT_BABY_STEP_DE;

  switch (gCfgItems.language) {
    case LANG_GERMAN:
      common_menu.dialog_confirm_title  = TITLE_DIALOG_CONFIRM_DE;
      common_menu.text_back             = BACK_TEXT_DE;
      common_menu.close_machine_tips    = DIALOG_CLOSE_MACHINE_DE;
      common_menu.unbind_printer_tips   = DIALOG_UNBIND_PRINTER_DE;
      common_menu.print_special_title   = PRINTING_DE;
      common_menu.pause_special_title   = PRINTING_PAUSE_DE;
      common_menu.operate_special_title = PRINTING_OPERATION_DE;
      //
      main_menu.title        = TITLE_READYPRINT_DE;
      main_menu.preheat      = PREHEAT_TEXT_DE;
      main_menu.move         = MOVE_TEXT_DE;
      main_menu.home         = HOME_TEXT_DE;
      main_menu.print        = PRINT_TEXT_DE;
      main_menu.extrude      = EXTRUDE_TEXT_DE;
      main_menu.leveling     = LEVELING_TEXT_DE;
      main_menu.autoleveling = AUTO_LEVELING_TEXT_DE;
      main_menu.fan          = FAN_TEXT_DE;
      main_menu.set          = SET_TEXT_DE;
      main_menu.more         = MORE_TEXT_DE;
      main_menu.tool         = TOOL_TEXT_DE;
      // TOOL
      tool_menu.title        = TOOL_TEXT_DE;
      tool_menu.preheat      = TOOL_PREHEAT_DE;
      tool_menu.extrude      = TOOL_EXTRUDE_DE;
      tool_menu.move         = TOOL_MOVE_DE;
      tool_menu.home         = TOOL_HOME_DE;
      tool_menu.leveling     = TOOL_LEVELING_DE;
      tool_menu.autoleveling = TOOL_AUTO_LEVELING_DE;
      tool_menu.filament     = TOOL_FILAMENT_DE;
      tool_menu.more         = TOOL_MORE_DE;
      //
      preheat_menu.adjust_title = TITLE_ADJUST_DE;
      preheat_menu.title        = TITLE_PREHEAT_DE;
      preheat_menu.add          = ADD_TEXT_DE;
      preheat_menu.dec          = DEC_TEXT_DE;
      preheat_menu.ext1         = EXTRUDER_1_TEXT_DE;
      preheat_menu.ext2         = EXTRUDER_2_TEXT_DE;
      preheat_menu.hotbed       = HEATBED_TEXT_DE;
      preheat_menu.off          = CLOSE_TEXT_DE;
      
      preheat_menu.step_1c     = TEXT_1C_DE;
      preheat_menu.step_5c     = TEXT_5C_DE;
      preheat_menu.step_10c    = TEXT_10C_DE;
      //
      move_menu.title = MOVE_TEXT_DE;
      //
      home_menu.title    = TITLE_HOME_DE;
      home_menu.stopmove = HOME_STOPMOVE_DE;
      //
      file_menu.title             = TITLE_CHOOSEFILE_DE;
      file_menu.page_up           = PAGE_UP_TEXT_DE;
      file_menu.page_down         = PAGE_DOWN_TEXT_DE;
      file_menu.file_loading      = FILE_LOADING_DE;
      file_menu.no_file           = NO_FILE_DE;
      file_menu.no_file_and_check = NO_FILE_DE;
      //
      extrude_menu.title       = TITLE_EXTRUDE_DE;
      extrude_menu.in          = EXTRUDER_IN_TEXT_DE;
      extrude_menu.out         = EXTRUDER_OUT_TEXT_DE;
      extrude_menu.ext1        = EXTRUDER_1_TEXT_DE;
      extrude_menu.ext2        = EXTRUDER_2_TEXT_DE;
      extrude_menu.low         = EXTRUDE_LOW_SPEED_TEXT_DE;
      extrude_menu.normal      = EXTRUDE_MEDIUM_SPEED_TEXT_DE;
      extrude_menu.high        = EXTRUDE_HIGH_SPEED_TEXT_DE;
      extrude_menu.temper_text = EXTRUDER_TEMP_TEXT_DE;      
      //
      leveling_menu.title     = TITLE_LEVELING_DE;
      leveling_menu.position1 = LEVELING_POINT1_TEXT_DE;
      leveling_menu.position2 = LEVELING_POINT2_TEXT_DE;
      leveling_menu.position3 = LEVELING_POINT3_TEXT_DE;
      leveling_menu.position4 = LEVELING_POINT4_TEXT_DE;
      leveling_menu.position5 = LEVELING_POINT5_TEXT_DE;
      //
      set_menu.title        = TITLE_SET_DE;
      set_menu.filesys      = FILESYS_TEXT_DE;
      set_menu.wifi         = WIFI_TEXT_DE;
      set_menu.about        = ABOUT_TEXT_DE;
      set_menu.fan          = FAN_TEXT_DE;
      set_menu.filament     = FILAMENT_TEXT_DE;
      set_menu.breakpoint   = BREAK_POINT_TEXT_DE;
      set_menu.motoroff     = MOTOR_OFF_TEXT_DE;
      set_menu.motoroffXY   = MOTOR_OFF_XY_TEXT_DE;
      set_menu.language     = LANGUAGE_TEXT_DE;
      set_menu.shutdown     = SHUTDOWN_TEXT_DE;
      set_menu.machine_para = MACHINE_PARA_DE;
      set_menu.eepromSet    = EEPROM_SETTINGS_DE;
      //
      filesys_menu.title   = TITLE_FILESYS_DE;
      filesys_menu.sd_sys  = SD_CARD_TEXT_DE;
      filesys_menu.usb_sys = U_DISK_TEXT_DE;
      //
      more_menu.title       = TITLE_MORE_DE;
      more_menu.gcode       = MORE_GCODE_DE;
      more_menu.entergcode  = MORE_ENTER_GCODE_DE;
      #if HAS_USER_ITEM(1)
        more_menu.custom1 = MORE_CUSTOM1_TEXT_DE;
      #endif
      #if HAS_USER_ITEM(2)
        more_menu.custom2 = MORE_CUSTOM2_TEXT_DE;
      #endif
      #if HAS_USER_ITEM(3)
        more_menu.custom3 = MORE_CUSTOM3_TEXT_DE;
      #endif
      #if HAS_USER_ITEM(4)
        more_menu.custom4 = MORE_CUSTOM4_TEXT_DE;
      #endif
      #if HAS_USER_ITEM(5)
        more_menu.custom5 = MORE_CUSTOM5_TEXT_DE;
      #endif
      #if HAS_USER_ITEM(6)
        more_menu.custom6 = MORE_CUSTOM6_TEXT_DE;
      #endif
      // WIFI
      wifi_menu.title = WIFI_TEXT;
      wifi_menu.cloud     = CLOUD_TEXT_DE;
      wifi_menu.reconnect = WIFI_RECONNECT_TEXT_DE;
      // CLOUD
      cloud_menu.title        = TITLE_CLOUD_TEXT_DE;
      cloud_menu.bind         = CLOUD_BINDED_DE;
      cloud_menu.binded       = CLOUD_BINDED_DE;
      cloud_menu.unbind       = CLOUD_UNBIND_DE;
      cloud_menu.unbinding    = CLOUD_UNBINDED_DE;
      cloud_menu.disconnected = CLOUD_DISCONNECTED_DE;
      cloud_menu.unbinded     = CLOUD_UNBINDED_DE;
      cloud_menu.disable      = CLOUD_DISABLE_DE;
      //
      about_menu.title   = ABOUT_TEXT_DE;
      about_menu.type    = ABOUT_TYPE_TEXT_DE;
      about_menu.version = ABOUT_VERSION_TEXT_DE;
      about_menu.wifi    = ABOUT_WIFI_TEXT_DE;

      //
      fan_menu.title = FAN_TEXT_DE;
      fan_menu.add   = FAN_ADD_TEXT_DE;
      fan_menu.dec   = FAN_DEC_TEXT_DE;
      fan_menu.state = FAN_TIPS1_TEXT_DE;
      //
      filament_menu.title                               = TITLE_FILAMENT_DE;
      filament_menu.in                                  = FILAMENT_IN_TEXT_DE;
      filament_menu.out                                 = FILAMENT_OUT_TEXT_DE;
      filament_menu.ext1                                = FILAMENT_EXT0_TEXT_DE;
      filament_menu.ext2                                = FILAMENT_EXT1_TEXT_DE;
      filament_menu.ready_replace                       = FILAMENT_CHANGE_TEXT_DE;
      filament_menu.filament_dialog_load_heat           = FILAMENT_DIALOG_LOAD_HEAT_TIPS_DE;
      filament_menu.filament_dialog_load_heat_confirm   = FILAMENT_DIALOG_LOAD_CONFIRM1_TIPS_DE;
      filament_menu.filament_dialog_loading             = FILAMENT_DIALOG_LOADING_TIPS_DE;
      filament_menu.filament_dialog_load_completed      = FILAMENT_DIALOG_LOAD_COMPLETE_TIPS_DE;
      filament_menu.filament_dialog_unload_heat         = FILAMENT_DIALOG_UNLOAD_HEAT_TIPS_DE;
      filament_menu.filament_dialog_unload_heat_confirm = FILAMENT_DIALOG_UNLOAD_CONFIRM_TIPS_DE;
      filament_menu.filament_dialog_unloading           = FILAMENT_DIALOG_UNLOADING_TIPS_DE;
      filament_menu.filament_dialog_unload_completed    = FILAMENT_DIALOG_UNLOAD_COMPLETE_TIPS_DE;

      //
      language_menu.title = TITLE_LANGUAGE_DE;
      language_menu.next  = PAGE_DOWN_TEXT_DE;
      language_menu.up    = PAGE_UP_TEXT_DE;

      //
      printing_menu.title  = TITLE_PRINTING_DE;
      printing_menu.option = PRINTING_OPERATION_DE;
      printing_menu.stop   = PRINTING_STOP_DE;
      printing_menu.pause  = PRINTING_PAUSE_DE;
      printing_menu.resume = PRINTING_RESUME_DE;

      //
      operation_menu.title      = TITLE_OPERATION_DE;
      operation_menu.pause      = PRINTING_PAUSE_DE;
      operation_menu.stop       = PRINTING_STOP_DE;
      operation_menu.temp       = PRINTING_TEMP_DE;
      operation_menu.fan        = FAN_TEXT_DE;
      operation_menu.filament   = FILAMENT_TEXT_DE;
      operation_menu.extr       = PRINTING_EXTRUDER_DE;
      operation_menu.speed      = PRINTING_CHANGESPEED_DE;
      operation_menu.more       = PRINTING_MORE_DE;
      operation_menu.move       = PRINTING_MOVE_DE;
      operation_menu.auto_off   = AUTO_SHUTDOWN_DE;
      operation_menu.manual_off = MANUAL_SHUTDOWN_DE;
      //
      pause_menu.title    = TITLE_PAUSE_DE;
      pause_menu.resume   = PRINTING_RESUME_DE;
      pause_menu.stop     = PRINTING_STOP_DE;
      pause_menu.extrude  = PRINTING_EXTRUDER_DE;
      pause_menu.move     = PRINTING_MOVE_DE;
      pause_menu.filament = FILAMENT_TEXT_DE;
      pause_menu.more     = PRINTING_MORE_DE;

      //
      speed_menu.title         = PRINTING_CHANGESPEED_DE;
      speed_menu.add           = ADD_TEXT_DE;
      speed_menu.dec           = DEC_TEXT_DE;
      speed_menu.move          = MOVE_SPEED_DE;
      speed_menu.extrude       = EXTRUDER_SPEED_DE;
      speed_menu.extrude_speed = EXTRUDER_SPEED_STATE_DE;
      speed_menu.move_speed    = MOVE_SPEED_STATE_DE;
      //
      printing_more_menu.title      = TITLE_MORE_DE;
      printing_more_menu.fan        = FAN_TEXT_DE;
      printing_more_menu.auto_close = AUTO_SHUTDOWN_DE;
      printing_more_menu.manual     = MANUAL_SHUTDOWN_DE;
      printing_more_menu.speed      = PRINTING_CHANGESPEED_DE;
      printing_more_menu.temp       = PRINTING_TEMP_DE;

      print_file_dialog_menu.confirm               = DIALOG_CONFIRM_DE;
      print_file_dialog_menu.cancel                = DIALOG_CANCLE_DE;
      print_file_dialog_menu.print_file            = DIALOG_PRINT_MODEL_DE;
      print_file_dialog_menu.cancel_print          = DIALOG_CANCEL_PRINT_DE;
      print_file_dialog_menu.retry                 = DIALOG_RETRY_DE;
      print_file_dialog_menu.stop                  = DIALOG_STOP_DE;
      print_file_dialog_menu.no_file_print_tips    = DIALOG_ERROR_TIPS1_DE;
      print_file_dialog_menu.print_from_breakpoint = DIALOG_REPRINT_FROM_BREAKPOINT_DE;

      print_file_dialog_menu.close_machine_error = DIALOG_ERROR_TIPS2_DE;
      print_file_dialog_menu.filament_no_press   = DIALOG_FILAMENT_NO_PRESS_DE;

      print_file_dialog_menu.print_finish     = DIALOG_PRINT_FINISH_DE;
      print_file_dialog_menu.print_time       = DIALOG_PRINT_TIME_DE;
      print_file_dialog_menu.reprint          = DIALOG_REPRINT_DE;
      print_file_dialog_menu.wifi_enable_tips = DIALOG_WIFI_ENABLE_TIPS_DE;
      print_file_dialog_menu.autolevelingTips   = DIALOG_AUTO_LEVELING_TIPS_DE;

      pause_msg_menu.pausing       = MESSAGE_PAUSING_DE;
      pause_msg_menu.changing      = MESSAGE_CHANGING_DE;
      pause_msg_menu.unload        = MESSAGE_UNLOAD_DE;
      pause_msg_menu.waiting       = MESSAGE_WAITING_DE;
      pause_msg_menu.insert        = MESSAGE_INSERT_DE;
      pause_msg_menu.load          = MESSAGE_LOAD_DE;
      pause_msg_menu.purge         = MESSAGE_PURGE_DE;
      pause_msg_menu.resume        = MESSAGE_RESUME_DE;
      pause_msg_menu.heat          = MESSAGE_HEAT_DE;
      pause_msg_menu.heating       = MESSAGE_HEATING_DE;
      pause_msg_menu.option        = MESSAGE_OPTION_DE;
      pause_msg_menu.purgeMore     = MESSAGE_PURGE_MORE_DE;
      pause_msg_menu.continuePrint = MESSAGE_CONTINUE_PRINT_DE;
      eeprom_menu.title            = EEPROM_SETTINGS_TITLE_DE;
      eeprom_menu.store            = EEPROM_SETTINGS_STORE_DE;
      eeprom_menu.read             = EEPROM_SETTINGS_READ_DE;
      eeprom_menu.revert           = EEPROM_SETTINGS_REVERT_DE;
      eeprom_menu.storeTips        = EEPROM_STORE_TIPS_DE;
      eeprom_menu.readTips         = EEPROM_READ_TIPS_DE;
      eeprom_menu.revertTips       = EEPROM_REVERT_TIPS_DE;
      break;
    ...
    default:
      ...
      cloud_menu.disable      = CLOUD_DISABLE_DE;
      //
      about_menu.title   = TITLE_ABOUT_DE;
      about_menu.type    = ABOUT_TYPE_TEXT_DE;
      about_menu.version = ABOUT_VERSION_TEXT_DE;
      about_menu.wifi    = ABOUT_WIFI_TEXT_DE;
      //
      fan_menu.title = TITLE_FAN_DE;
      fan_menu.add   = FAN_ADD_TEXT_DE;
      fan_menu.dec   = FAN_DEC_TEXT_DE;
      fan_menu.state = FAN_TIPS1_TEXT_DE;
      //
      ...
      //
      speed_menu.title         = TITLE_CHANGESPEED_DE;
      speed_menu.add           = ADD_TEXT_DE;
      speed_menu.dec           = DEC_TEXT_DE;
      speed_menu.move          = MOVE_SPEED_DE;
      speed_menu.extrude       = EXTRUDER_SPEED_DE;
