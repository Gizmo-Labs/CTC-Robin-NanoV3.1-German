...
uint32_t Pic_Info_Write(uint8_t *P_name, uint32_t P_size) {
  ...
  static void loadAsset(SdFile &dir, dir_t& entry, const char *fn, int8_t assetType) {
    ...
    watchdog_refresh();
    // Zeigt den Dateinamen des Assets an...
  ...
  void UpdateAssets() {
    ...
    if (dir.open(&root, assetsPath, O_RDONLY)) {
      ...
      disp_assets_update();
      disp_assets_update_progress("Entferne Bilder...");
      watchdog_refresh();
      spiFlashErase_PIC();
      #if HAS_SPI_FLASH_FONT
        disp_assets_update_progress("Entferne Schriften...");
        watchdog_refresh();
        spiFlashErase_FONT();
      #endif

      disp_assets_update_progress("Lese Dateien in Verzeichnis...");
      dir_t d;
      while (dir.readDir(&d, card.longFilename) > 0) {
        disp_assets_update_progress("Lese Datei...");
        ...
        if (a >= 0 && a < (int8_t)COUNT(assets)) 
        {
          ...
          // Kurze Pause zwischen Dateien, damit Dateiname lesbar ist...
          delay(500);
      ...
      disp_assets_update_progress("Schreibvorgang beendet...");
    }
    dir.close();
