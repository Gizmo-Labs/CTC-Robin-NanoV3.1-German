...
extern lv_group_t *g;
static lv_obj_t *scr;
static lv_obj_t *buttonDE, *buttonT_CN, *buttonEN, *buttonRU;
...
static void event_handler(lv_obj_t *obj, lv_event_t event) {
  ...
  switch (obj->mks_obj_id) {
    case ID_DE:
      ...
      lv_imgbtn_set_src_both(buttonDE, "F:/bmp_german.bin");
      lv_obj_refresh_ext_draw_pad(buttonDE);
...
static void disp_language(uint8_t language, uint8_t state) {
  ...
  switch (language) {
    case LANG_GERMAN:
      id = ID_DE;
      strcpy_P(public_buf_l, PSTR("F:/bmp_german"));
      obj = buttonDE;
      break;
...
void lv_draw_language() {
  ...
  buttonDE = lv_big_button_create(scr, "F:/bmp_german.bin", language_menu.german, INTERVAL_V, titleHeight, event_handler, ID_DE);
  lv_obj_clear_protect(buttonDE, LV_PROTECT_FOLLOW);
