...
void gCfgItems_init()
{
  ...
  gCfgItems.trammingPos[0].x = X_MIN_POS + 10;
  gCfgItems.trammingPos[0].y = Y_MIN_POS + 10;
  gCfgItems.trammingPos[1].x = X_MAX_POS - 40;
  gCfgItems.trammingPos[1].y = Y_MAX_POS + 10;
  gCfgItems.trammingPos[2].x = X_MAX_POS - 40;
  gCfgItems.trammingPos[2].y = Y_MAX_POS - 10;
  gCfgItems.trammingPos[3].x = X_MIN_POS + 10;
  gCfgItems.trammingPos[3].y = Y_MAX_POS - 10;
  ...
  gCfgItems.filamentchange_load_speed = 300;
  gCfgItems.filamentchange_unload_length = 200;
  gCfgItems.filamentchange_unload_speed = 300;
...
void tft_style_init()
{
  ...
  style_sel_text.text.color = LV_COLOR_BLUE;
  style_sel_text.text.sel_color = LV_COLOR_BLUE;
...
char *getDispText(int index)
{
  ...
  case PRINTING_UI:
    ...
    {
      IF_DISABLED(TFT35, case OPERATE_UI
                  : case PAUSE_UI:)
    case PRINTING_UI:
  ...
  case TOOL_UI:
    ...
    break;
  case WIFI_LIST_UI:
    TERN_(MKS_WIFI_MODULE, strcpy(public_buf_l, list_menu.title));
    break;
...
void GUI_RefreshPage()
{
  if ((systick_uptime_millis % 1000) == 0)
    temps_update_flag = true;
  if ((systick_uptime_millis % 3000) == 0)
    printing_rate_update_flag = true;
...
void clear_cur_ui()
{
  ...
  case ACCELERATION_UI:
    ...
    break;
  case JERK_UI:
    TERN_(HAS_CLASSIC_JERK, lv_clear_jerk_settings());
    break;
  case MOTORDIR_UI:
    break;
  case HOMESPEED_UI:
...
lv_obj_t *lv_imgbtn_create(lv_obj_t *par, const char *img, lv_event_cb_t cb, const int id /*=0*/)
{
  ...
  if (img)
    lv_imgbtn_set_src_both(btn, img);
  if (id)
    lv_obj_set_event_cb_mks(btn, cb, id, "", 0);
  else
    lv_obj_set_event_cb(btn, cb);
